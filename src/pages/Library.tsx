...
const Library = () => {
  ...
  const renderContent = () => {
    if (isLoading && libraryItems.length === 0) {
      return renderSkeletonItems(viewMode);
    }
  ...
  const renderSkeletonItems = (mode: "grid" | "list") => {
    const items = Array.from({ length: 9 }).map((_, index) => ({ key: `skeleton-${mode}-${index}` }));

    if (mode === "grid") {
      return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-6">
          {items.map((item) => (
            <div key={item.key} className="bg-card rounded-lg shadow-card overflow-hidden">
    ...
    return (
      <div className="space-y-2 mt-6">
        {items.map((item) => (
          <div key={item.key} className="bg-card rounded-lg shadow-card overflow-hidden flex">
